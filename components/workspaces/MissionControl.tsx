...
/**
 * ICON REGISTRY - Helper function to ensure single parent element for SVG components.
 * This prevents the TS2657 error during the 'tsc' build step.
 */
...
export const MissionControl: React.FC<MissionControlProps> = ({ leads, theater, onNavigate }) => {
  const stats = [
    { label: 'API NODES', status: 'STABLE', icon: <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" />, color: 'emerald' },
    { label: 'IDENTIFIED', status: `${leads.length} TARGETS`, icon: <ControlIcon type="IDENTIFIED" />, color: 'indigo' },
    { label: 'THREAT SCAN', status: 'OPTIMAL', icon: <path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" />, color: 'cyan' },
    { label: 'OS VERSION', status: 'V13.1.2', icon: <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />, color: 'slate' },
  ];

  const actions = [
    { id: 'RADAR_RECON', mode: 'OPERATE' as MainMode, title: 'RADAR RECON', desc: 'INITIATE THEATER SCAN', icon: <ControlIcon type="RADAR" />, theme: 'indigo' },
    { id: 'TARGET_LIST', mode: 'OPERATE' as MainMode, title: 'TARGET LEDGER', desc: 'DATABASE ACCESS', icon: <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20M4 4.5A2.5 2.5 0 0 1 6.5 7H20v10H6.5" />, theme: 'emerald' },
    { id: 'VIDEO_PITCH', mode: 'CREATE' as MainMode, title: 'PAYLOAD FORGE', desc: 'GENERATE ASSETS', icon: <ControlIcon type="PAYLOAD" />, theme: 'cyan' }
  ];
  ...
  return (
    <div className="space-y-10 py-4 max-w-6xl mx-auto animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((node, i) => (
          <div key={i} className="bg-[#0b1021]/60 border border-slate-800/80 p-5 rounded-2xl flex flex-col items-center group hover:border-indigo-500/40 transition-all cursor-default">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
        {actions.map((action, i) => (
          <div 
            ...
            className="bg-[#0b1021] border border-slate-800/80 p-10 rounded-[48px] relative overflow-hidden group hover:border-indigo-600/40 transition-all cursor-pointer shadow-xl hover:bg-slate-900/40 group flex flex-col items-center text-center"
          >
