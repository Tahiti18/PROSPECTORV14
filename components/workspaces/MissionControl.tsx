...
export const MissionControl: React.FC<MissionControlProps> = ({ leads, theater, onNavigate }) => {
  return (
    <div className="space-y-10 py-4 max-w-6xl mx-auto">
      ...
      {/* Pro Stats HUD - No Red */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'API NODES', status: 'STABLE', icon: <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" />, color: 'emerald' },
          { label: 'IDENTIFIED', status: `${leads.length} TARGETS`, icon: <circle cx="12" cy="12" r="10" /><circle cx="12" cy="12" r="6" />, color: 'indigo' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
        {[
          { id: 'RADAR_RECON', mode: 'OPERATE' as MainMode, title: 'RADAR RECON', desc: 'INITIATE THEATER SCAN', icon: <circle cx="12" cy="12" r="10" /><path d="M12 2v20M2 12h20" />, theme: 'indigo' },
          ...
          { id: 'VIDEO_PITCH', mode: 'CREATE' as MainMode, title: 'PAYLOAD FORGE', desc: 'GENERATE ASSETS', icon: <path d="M23 7l-7 5 7 5V7z" /><rect x="1" y="5" width="15" height="14" rx="2" ry="2" />, theme: 'cyan' }
        ].map((action, i) => (
          <div 
            key={i}
