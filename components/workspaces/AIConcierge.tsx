  ...
  return (
    <div className="max-w-5xl mx-auto py-8 space-y-12 animate-in fade-in duration-500">
      <div className="flex justify-between items-end border-b border-slate-800/50 pb-8">
        <div>
          <h1 className="text-5xl font-black italic text-white uppercase tracking-tighter">AI <span className="text-indigo-600 not-italic">CONCIERGE</span></h1>
          <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.4em] mt-2 italic italic">Autonomous Agent POC: {lead.businessName}</p>
        </div>
      </div>
           ...
           <div className="bg-[#0b1021] border border-slate-800 rounded-[32px] p-8 shadow-2xl space-y-8">
              <div className="space-y-2">
                 ...
                 <textarea 
                  ...
                  className="w-full bg-[#020617] border border-slate-800 rounded-2xl p-5 text-[11px] font-bold text-slate-300 focus:outline-none focus:border-indigo-500 h-32 resize-none shadow-xl italic"
                 />
              </div>
              <button 
           ...
           <div className="flex-1 overflow-y-auto space-y-6 custom-scrollbar pr-4">
              {chat.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-start' : 'justify-end'}`}>
                   <div className={`max-w-[80%] p-6 rounded-[28px] text-[12px] font-medium leading-relaxed italic ${m.role === 'user' ? 'bg-slate-900 border border-slate-800 text-slate-400 rounded-bl-none' : 'bg-indigo-600/10 border border-indigo-500/20 text-indigo-400 rounded-br-none'}`}>
              ...
              {isLoading && <div className="text-[10px] font-black text-indigo-500 animate-pulse uppercase tracking-[0.4em] text-center py-20">AGENT_THINKING...</div>}
