...
export const OutreachModal: React.FC<OutreachModalProps> = ({ isOpen, onClose, dossier, lead, onUpdateLead, onSent }) => {
  ...
  const getHtmlPreview = () => {
    return `
      <!DOCTYPE html>
      <html>
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding: 20px; color: #1e293b; line-height: 1.6;">
        <div style="max-width: 600px; margin: 0 auto;">
          <h2 style="font-size: 18px; font-weight: 700; margin-bottom: 20px; color: #0f172a; border-bottom: 1px solid #e2e8f0; padding-bottom: 10px;">
            Subject: ${subject}
          </h2>
          <div style="font-size: 14px; white-space: pre-wrap;">${body}</div>
          <div style="margin-top: 40px; font-size: 10px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.1em; border-top: 1px solid #f1f5f9; padding-top: 20px;">
  ...
  const handleSendToMyself = async () => {
    if (!adminEmail || !adminEmail.includes('@')) {
      alert("Please configure a valid Admin Email in TEST mode first.");
      return;
    }
    
    setIsSending(true);
    
    // Auto-copy if body is long (Gmail URL limit safeguard)
    if (body.length > 1800) {
      await outreachService.copyToClipboard(body);
      alert("Body is long (>1800 chars). Text copied to clipboard. Paste into Gmail after opening.");
    }

    const mailto = outreachService.generateMailto(adminEmail, `[TEST] ${subject}`, body);
    window.open(mailto, '_blank');

    outreachService.logInteraction(lead.id, 'EMAIL', subject, 'test', adminEmail);
               ...
               <div className="flex gap-2">
                 ...
                 {channel === 'GMAIL' && (
                   <button 
                     ...
                     title="Send test to Admin Email"
                   >
                     <span>🧪</span> ME
                   </button>
                 )}
