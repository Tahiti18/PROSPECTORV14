      ...
      <div className="bg-[#0b1021] border border-slate-800 rounded-[40px] p-12 shadow-2xl grid grid-cols-1 md:grid-cols-2 gap-12">
        <div className="space-y-8">
          <div className="space-y-3">
             ...
             <input type="number" value={ltv} onChange={e => setLtv(parseInt(e.target.value) || 0)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-5 py-4 text-white font-black text-xl focus:outline-none focus:border-indigo-500 transition-colors" />
          </div>
          <div className="space-y-3">
             <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Monthly Lead Volume</label>
             <input type="number" value={leadsCount} onChange={e => setLeadsCount(parseInt(e.target.value) || 0)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-5 py-4 text-white font-black text-xl focus:outline-none focus:border-indigo-500 transition-colors" />
          </div>
          <div className="space-y-3">
             <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">AI Conv. Lift (%)</label>
             <input type="range" min="0.5" max="15" step="0.5" value={convRate} onChange={e => setConvRate(parseFloat(e.target.value))} className="w-full accent-indigo-600" />
             <div className="flex justify-between text-[10px] font-black text-indigo-400 uppercase tracking-widest px-1"><span>{convRate}% TARGETED LIFT</span></div>
          </div>
        </div>
        ...
        <div className="bg-[#05091a] rounded-[32px] border border-slate-800/60 p-10 flex flex-col items-center justify-center text-center space-y-2 relative overflow-hidden">
           ...
           <button 
             ...
             className="mt-8 bg-indigo-600/10 border border-indigo-500/20 text-indigo-400 px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all relative z-10 disabled:opacity-50"
           >
