  ...
  return (
    <div className="max-w-5xl mx-auto py-8 space-y-12 animate-in fade-in duration-500">
      <div className="text-center">
        <h1 className="text-5xl font-black italic text-white uppercase tracking-tighter">FACT <span className="text-indigo-600 not-italic">CHECK</span></h1>
      ...
      <div className="bg-[#0b1021] border border-slate-800 rounded-[40px] p-12 space-y-10 shadow-2xl">
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            <input 
              ...
              className="flex-1 bg-slate-950 border border-slate-800 rounded-2xl px-6 py-5 text-white font-bold text-sm focus:outline-none focus:border-indigo-500 transition-all shadow-inner"
              placeholder="e.g. They recently expanded to a second location, They are struggling with bad PR..."
            />
            <button 
              onClick={handleVerify}
              disabled={isLoading || !claim}
              className="bg-indigo-600 hover:bg-indigo-500 px-10 rounded-2xl text-[11px] font-black uppercase tracking-widest text-white transition-all shadow-xl shadow-indigo-600/20 active:scale-95 disabled:opacity-50"
            >
        ...
        {isLoading && (
          <div className="h-64 bg-slate-950/50 border border-slate-800 border-dashed rounded-[32px] flex flex-col items-center justify-center space-y-6">
             ...
             <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] animate-pulse">Consulting Grounded Search Vectors...</p>
          </div>
        )}
             ...
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   {result.sources?.map((s: any, i: number) => (
                     <a key={i} href={s.uri} target="_blank" rel="noopener noreferrer" className="p-5 bg-slate-900 border border-slate-800 rounded-2xl hover:border-indigo-500/40 transition-all group">
                        <p className="text-[10px] font-black text-white uppercase tracking-widest truncate group-hover:text-indigo-400 transition-colors">{s.title}</p>
