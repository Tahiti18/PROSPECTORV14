      ...
      <div className="bg-white dark:bg-[#0b1021] border border-slate-200 dark:border-indigo-500/30 rounded-[48px] p-12 shadow-2xl space-y-10 relative overflow-hidden transition-colors">
         ...
         <div className="flex flex-col md:flex-row justify-around items-center space-y-8 md:space-y-0">
           <div className="text-center space-y-1">
              <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest italic">LOCAL SESSION ESTIMATE</p>
              <p className="text-4xl font-black italic text-slate-900 dark:text-white tracking-tighter">${compute?.sessionCostUsd.toFixed(2) || '0.00'}</p>
           </div>
           <div className="w-px h-12 bg-slate-200 dark:bg-slate-800 hidden md:block"></div>
           <div className="text-center space-y-1">
              <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest italic">ELITE_PRO CALLS</p>
              <p className="text-4xl font-black italic text-indigo-600 dark:text-indigo-400 tracking-tighter">{compute?.proCalls || 0}</p>
           </div>
           <div className="w-px h-12 bg-slate-200 dark:bg-slate-800 hidden md:block"></div>
           <div className="text-center space-y-1">
              <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest italic">EST. MONTHLY OPEX</p>
              <p className={`text-4xl font-black italic tracking-tighter ${compute && compute.projectedMonthlyUsd > 100 ? 'text-amber-600 dark:text-amber-500' : 'text-emerald-600 dark:text-emerald-400'}`}>
                ${compute?.projectedMonthlyUsd.toFixed(2) || '0.00'}
              </p>
           </div>
         </div>

         <div className="pt-8 border-t border-slate-100 dark:border-slate-800/50 flex flex-col items-center gap-8">
            <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-500/20 p-6 rounded-2xl max-w-2xl text-center">
              <p className="text-[10px] text-amber-800 dark:text-amber-400 font-bold uppercase tracking-widest leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
              <a 
                ...
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-10 py-5 rounded-[24px] text-[10px] font-black uppercase tracking-[0.3em] transition-all shadow-xl shadow-indigo-600/30 active:scale-95 border border-indigo-400/20 flex items-center justify-center gap-3"
              >
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="bg-slate-900 dark:bg-slate-800 hover:bg-black text-white px-10 py-5 rounded-[24px] text-[10px] font-black uppercase tracking-[0.3em] transition-all shadow-xl active:scale-95 border border-slate-700 flex items-center justify-center gap-3"
              >
