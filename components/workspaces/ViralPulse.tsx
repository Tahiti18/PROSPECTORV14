      ...
      <div className="flex justify-between items-start">
        <div className="space-y-2">
          <h1 className="text-4xl font-black italic text-white uppercase tracking-tighter flex items-center gap-3">
            VIRAL <span className="text-indigo-500">PULSE AGENT</span>
        ...
        <div className="bg-indigo-600/10 border border-indigo-500/20 px-6 py-2.5 rounded-full flex items-center gap-3">
           ...
           <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em]">GLOBAL TRENDS FEED LIVE</span>
        </div>
      </div>
        ...
        <div className="lg:col-span-4 space-y-8">
           <div className="bg-[#0b1021] border border-slate-800 rounded-[48px] p-10 shadow-2xl space-y-10">
                 ...
                 {trends.slice(0, 4).map((t, i) => (
                   <div key={i} className="space-y-3">
                      ...
                      <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                         <div className={`h-full rounded-full ${t.type === 'up' ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-indigo-600 shadow-[0_0_10px_rgba(99,102,241,0.5)]'}`} style={{ width: `${(t.val/150)*100}%` }}></div>
                      </div>
                   </div>
                 ))}
              ...
              <button 
                ...
                className="w-full bg-slate-900 border border-slate-800 hover:border-indigo-500/40 text-white py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] transition-all"
              >
                 REFRESH TRENDS
              </button>
           </div>
